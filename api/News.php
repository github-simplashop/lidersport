...
class News extends Simpla
{
	...
	*/
	...
	{	
		...
		$query = $this->db->placehold("SELECT b.id, b.url, b.name, b.annotation, b.text,
		                                      ...
		                                      b.date
		                                      FROM __news b WHERE 1 $post_id_filter $visible_filter $keyword_filter
	...
	*/	
	...
	{
		$this->db->query("SELECT date FROM __news WHERE id=? LIMIT 1", $id);
		$date = $this->db->result('date');

		$this->db->query("(SELECT id FROM __news WHERE date=? AND id>? AND visible  ORDER BY id limit 1)
		                   UNION
		                  (SELECT id FROM __news WHERE date>? AND visible ORDER BY date, id limit 1)",
		                  $date, $id, $date);
	...
	*/	
	public function get_prev_post($id)
	{
		...
		$this->db->query("(SELECT id FROM __news WHERE date=? AND id<? AND visible ORDER BY id DESC limit 1)
		                   UNION
		                  (SELECT id FROM __news WHERE date<? AND visible ORDER BY date DESC, id DESC limit 1)",
		                  $date, $id, $date);
