	...
	*/
	public function get_post($id)
	{
		...
		$query = $this->db->placehold("SELECT b.id, b.url, b.name, b.annotation, b.text, b.meta_title,
		                               b.meta_keywords, b.meta_description, b.visible, b.date, b.video,b.image
	...
	{
		$query = $this->db->placehold("SELECT image FROM __blog WHERE id=?", $id);
		$this->db->query($query);
		...
		if(!empty($filename))
		{
			...
			if($count == 0)
			{			
				@unlink($this->config->root_dir.$this->config->stock_images_dir.$filename);		
			}
		}
	}
	/*
