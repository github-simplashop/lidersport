...
class Notify extends Simpla
{
	...
	public function email_order_admin($order_id)
	{
            ...
            $shop = $this->delivery->get_shop($order->shop_id);
			...
			// Отправляем письмо
			$email_template = $this->design->fetch($this->config->root_dir.'madmin/design/html/email_order_admin.tpl');
			$subject = $this->design->get_var('subject');
			$this->email($this->settings->order_email, $subject, $email_template, $this->settings->notify_from_email);
			$this->email("om@lidsport.ru", $subject, $email_template, $this->settings->notify_from_email);
	}
	...
	{ 
			...
			// Отправляем письмо
			$email_template = $this->design->fetch($this->config->root_dir.'madmin/design/html/email_comment_admin.tpl');
			$subject = $this->design->get_var('subject');
	...
	{ 
			...
			// Отправляем письмо
			$email_template = $this->design->fetch($this->config->root_dir.'madmin/design/html/email_feedback_admin.tpl');
			$subject = $this->design->get_var('subject');
