...
class Comments extends Simpla
{
	...
	{
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.name, c.email, c.ip, c.type, c.text, c.rating, c.date, c.approved FROM __comments c WHERE id=? LIMIT 1", intval($id));
...
//			$approved_filter = $this->db->placehold("AND (c.approved=? $ip)", intval($filter['approved']));
		...
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.ip, c.name, c.email, c.text, c.rating, c.type, c.date, c.text, c.approved
