...

{
...

	{
		...
		$query = $this->db->placehold("SELECT id, delivery_id, shop_id, delivery_price, separate_delivery, payment_method_id, paid, payment_date, closed, discount, date,
...

	{
		...
		if(isset($_SESSION['user_id']) && $user = $this->users->get_user(intval($_SESSION['user_id']))){
			if (isset($variant->compare_price) || $variant->compare_price!=""){
				$purchase->price = $variant->price;
			}	
			else
			{
				$purchase->price = $variant->price*(100-$order->discount)/100;
			}
		}
		else
		{
...

	{
		...
		if(empty($order))
	 	...
	 	$query = $this->db->placehold("UPDATE __orders o SET o.total_price=IFNULL((SELECT SUM(p.price*p.amount) FROM __purchases p WHERE p.order_id=o.id), 0)+o.delivery_price*(1-o.separate_delivery)-o.coupon_discount, modified=NOW() WHERE o.id=? LIMIT 1", $order->id);
