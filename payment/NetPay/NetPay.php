...
class NetPay extends Simpla {
    ...
    public function checkout_form($order_id, $button_text = null) {
        ...
        if (!array_key_exists('test_mode', $payment_settings))
                $payment_settings['test_mode'] = '0';
        ...
        if (!empty($payment_settings['inn'])) {
			...
			$NetpayAtol = new NetpayAtol(
				$payment_settings['inn'],
                                $params['email'],
                                $order->total_price,
                                '',
                                $this->phone,
                                $payment_settings['test_mode'] == '1'
			);
			...
			foreach ($purchases as $product) {
				$NetpayAtol->addItem(
                                    ...
                                    isset($product->tax) ? $product->tax : $this->tax,
                                    isset($product->tax_sum) ? $product->tax_sum : null
                                );
			}
                        ...
                        $NetpayAtol->addShipping(
                                empty($dl->name) ? 'Доставка': $dl->name, 
                                number_format($payment_settings['delivery_price'], 2), 
                                null
                        );
			
			$params['cashbox_data'] = $NetpayAtol->getJSON();
			//$this->errors = array_merge($this->errors, $NetpayAtol->getErrors());
