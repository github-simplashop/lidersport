...
class NetPay extends Simpla {
    ...
    public function checkout_form($order_id, $button_text = null) {
        ...
        if ($payment_settings['test_mode'] == '1'){
		...
		// ATOL include  		
        if (!empty($payment_settings['inn'])) {
			$r = new CashRegisterRequest();
			$r->setInn($payment_settings['inn']);
			$r->setPaymentAddress($_SERVER['HTTP_HOST']);
			$r->setAmount($order->total_price);
			$r->setPurchaserEmail($params['email']);
			$r->setPurchaserPhone('');
			
			// Product list
			$purchases = $this->orders->get_purchases(array('order_id'=>$order->id));
			foreach ($purchases as $item) {
				$p = new CashRegisterRequestProduct();
				$p->setName($item->product_name);		// your product name
				$p->setPrice($item->price);	// your product price
				$p->setCount($item->amount);		// count of products
				$p->setAmount($item->price * $item->amount);
				$p->setTaxByNumber($payment_settings['tax']);
				$r->addProduct($p);		
			}
			
			// shipment like product
			if ((float)$payment_settings['delivery_price'] > 0) {
				$dl = $this->delivery->get_delivery($order->delivery_id);
				$p = new CashRegisterRequestProduct();
			    $p->setName(empty($dl['name']) ? 'Доставка': $dl['name']);
			    $p->setPrice(number_format($payment_settings['delivery_price'], 2));
			    $p->setCount(1);
			    $p->setAmount(number_format($payment_settings['delivery_price'], 2));
			    $p->setTaxByNumber('NULL');
			    $r->addProduct($p);    
			}
			
			// Final sum
			$r->addPayment($params['amount']);
			
			$params['cashbox_data'] = $r->toTextFormat();
			
			// DEBUG
			//file_put_contents(dirname(__FILE__).'/cashbox.log', $params['cashbox_data']."\n\n");
		} 
