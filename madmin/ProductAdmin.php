...
class ProductAdmin extends Simpla
{
	...
	private $allowed_files_extentions = array('png', 'jpg');
	...
	public function fetch()
	{
	   $product = new StdClass();
		...
		if($this->request->method('post') && !empty($_POST))
		{
			...
			$product->harakt = $this->request->post('harakt');
			$product->video = $this->request->post('video');
           
			$product->content_title = $this->request->post('content_title');
	 	  		    ...
	 	  		    if(is_array($related_products))
		  		    {
		  		    	...
		  		    	foreach($related_products  as $i=>$related_product)
	   	    				$this->products->add_related_product($product->id, $related_product->related_id, $pos++);
                                        ...
                                        // Теги
                           $this->products->delete_tags('product', $product->id);
                           $this->products->add_tags('product', $product->id, $this->request->post('tags'));
        ...
        //Вытягиваем цвет из имени варианта
        ...
        foreach($parsevariants as $pv){
            ...
            $color[$pv->id] = $this->colors->toggle_color($colors[$pv->id]);   
        }    
            
        $this->design->assign('variant_color', $color);    
