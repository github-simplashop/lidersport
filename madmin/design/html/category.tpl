...
$(function() {
	...
	$(".podobrat a.delete_podobrat").click( function() {

		$("input[name='delete_image_podobrat']").val('1');
...

{
	...
	if(!meta_title_touched)
	{
		...
		$('input[name="content_title"]').val(generate_meta_title());
	}
...
<div class="message message_success">
	...
	<a class="link" target="_blank" href="/{$category->url}">Открыть категорию на сайте</a>
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				...
				<li><label class=property>Адрес</label><div class="page_url">/</div><input name="url" class="page_url" type="text" value="{$category->url|escape}" /></li>

				<li><label class=property>H1 Заголовок</label><input name="content_title" class="simpla_inp" type="text" value="{$category->content_title|escape}" /></li>
                
				<li><label class=property>M Заголовок</label><input name="meta_title" class="simpla_inp" type="text" value="{$category->meta_title|escape}" /></li>

				<li><label class=property>M Ключевые слова</label><input name="meta_keywords" class="simpla_inp" type="text" value="{$category->meta_keywords|escape}" /></li>

				<li><label class=property>M Описание</label><textarea name="meta_description" class="simpla_inp" />{$category->meta_description|escape}</textarea></li>
                
                <li><label class=property>Отображение свойств плиткой</label><input type="checkbox" name="plitka" value="1" {if $category->plitka==1}checked{/if} /></li>
		...
		<!-- Параметры страницы (The End)-->
                ...
                <div class="block layer">
        	<h2>Теги</h2>
        	<input type="text" name="tags" value="{foreach $category->tags as $tag}{$tag->value|escape}{if !$tag@last},{/if}{/foreach}" />
        	{literal}
        	<script>
	...
	<div id="column_right">
		...
		<div class="block layer images podobrat">

			<h2>Изображение для "как подобрать размер?"</h2>

			<input class='upload_image_podobrat' name="image_podobrat" type="file">			

			<input type="hidden" name="delete_image_podobrat" value="">

			{if $category->podobrat}

			<ul>

				<li style="line-height: initial;height: initial;">

					<a href='#' class="delete_podobrat"><img src='design/images/cross-circle-frame.png'></a>

					<img src="../{$config->categories_images_dir}{$category->podobrat}" alt="" />
