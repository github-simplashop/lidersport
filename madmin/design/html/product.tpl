...
$(function() {
  ...
  function show_category_features(category_id) {
      ...
      if (categories_features[category_id] !== undefined) {
          $('ul.prop_ul li').filter(function () {
              return jQuery.inArray($(this).attr("feature_id"), categories_features[category_id]) > -1;
          }).show();
      }
  }
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	...
	<div id="variants_block" {assign var=first_variant value=$product_variants|@first}{if $product_variants|@count <= 1 && !$first_variant->name}class=single_variant{/if}>
		...
		<ul>
            ...
            <li class="variant_color">
                    ...
                    <label ><input disabled="true" style="{if $variant_color[$variant->id]->textura}
                    background: url('../../images/textures/{$variant_color[$variant->id]->textura}') no-repeat 10%;"/>
                    {elseif $variant_color[$variant->id]->html1}background-color: {$variant_color[$variant->id]->html1}"/> 
                    {else}background: url('design/images/cross-circle-frame.png') no-repeat 10px;"/>{/if}</label>
		...
		<ul id=new_variant style='display:none;'>
            ...
            <li class="variant_color">
                <div>
                    ...
                    <input class="color1" name="variants[color][]" type="" value="" />
                </div>
            </li>
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				...
				<li><label class=property>H1 Заголовок</label><input name="content_title" class="simpla_inp" type="text" value="{$product->content_title|escape}" /></li>
                <li><label class=property>M Заголовок</label><input name="meta_title" class="simpla_inp" type="text" value="{$product->meta_title|escape}" /></li>
				<li><label class=property>M Ключевые слова</label><input name="meta_keywords" class="simpla_inp" type="text" value="{$product->meta_keywords|escape}" /></li>
				<li><label class=property>M Описание</label><textarea name="meta_description" class="simpla_inp" />{$product->meta_description|escape}</textarea></li>
                <li><label class=property>Видео</label><input name="video" class="simpla_inp" type="text" value="{$product->video|escape}" /></li>
			</ul>
		</div>
		<!-- Параметры страницы (The End)-->
                ...
                <div class="block layer">
        	<h2>Теги</h2>
        	<input type="text" name="tags" value="{foreach $product->tags as $tag}{$tag->value|escape}{if !$tag@last},{/if}{/foreach}" />
        	{literal}
        	<script>
    ...
    <div class="block">		
		...
		<textarea name="harakt" class="editor_large">{$product->harakt|escape}</textarea>
	</div>
