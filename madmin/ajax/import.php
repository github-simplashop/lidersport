...
{	
	...
	private $import_files_dir      = 'madmin/files/import/'; // Временная папка		
	...
	private function import_item($item)
	{
		...
		else $product['meta_title'] = trim($item['name']);
		...
		elseif(isset($item['name']))
			...
			$product['url'] = $this->translit($item['name']);
			$product['url'] = str_ireplace("/","_",$product['url']);
			$product['url'] = str_ireplace(" ","_",$product['url']);
		...
		if(isset($item['price']))
			...
			$variant['price'] = str_ireplace(" ","",$variant['price']);
