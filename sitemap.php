...
foreach($simpla->categories->get_categories() as $c)
{
	if($c->visible)
	{
		$url = $simpla->config->root_url.'/'.esc($c->url);
		print "\t<url>"."\n";
		print "\t\t<loc>$url</loc>"."\n";
		print "\t</url>"."\n";
	}
...
// Товары
$simpla->db->query("SELECT url,id FROM __products WHERE visible=1");
foreach($simpla->db->results() as $p)
{
	$url2 = $simpla->categories->get_product_categories_url($p->id);
	
	$url = $simpla->config->root_url.'/'.$url2.'/u_'.esc($p->url);
	print "\t<url>"."\n";
	print "\t\t<loc>$url</loc>"."\n";
	print "\t</url>"."\n";
}
