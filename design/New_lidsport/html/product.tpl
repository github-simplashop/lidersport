...
{/if}
	...
	<div class="wrapper">
		...
		<div class="window window__product">
			...
			<div class="card">
				...
				<div class="card__right">
					<h1 class="card__title">{$product->name|escape}</h1>
                                        ...
                                        {*if $product->tags}
                                                <p class="tags">
                                                        ...
                                                        <a href="tags/{$tag->value|escape}">{$tag->value|escape}</a>{if !$tag@last}, {/if}
                                                        {/foreach}
                                                </p>
                                                {/if*}
            ...
            {elseif $product->variants|count > 0}
                ...
                {if $product->variants|count > 0}
                  ...
                  {/if}
            {/if}
                                {if $category->name == "Самокаты для детей" 
                                || $category->name == "2-х и 4-х колесные" 
                                || $category->name == "3-х колесные" 
                                || $category->name == "Беговелы" 
                                || $category->name == "Бибикары"
                                || $category->name == "Подростковые"
                                || $category->name == "Женские"
                                || $category->name == "Городские и дорожные"
                                || $category->name == "Горные"}
                                <a href="#accordion__delivery_img" id="scroll__delivery_how" class="card__how how_to_choose">Как выбрать размер?</a>
                        ...
                        {if $empty_razmer}
							...
							<div id="variants" class="card__block card__block--size {if $empty_razmer}active{/if} {if $category->plitka}point{else}select{/if}">
                                ...
                                {if $category->plitka}
                                    ...
                                    {/foreach}
										{else}
											<select class="js-select" name="razmers">
							...
							<div class="card__info-block">
								...
								<div class="card__amount">
									{if $product->variant->stock > 5} 
									  Осталось более 5 товаров
									{else}
                    {$product->variant->stock|plural:'Остался':'Осталось'} всего {$product->variant->stock} {$product->variant->stock|plural:'товар':'товара'}
                  {/if}
								...
								{if $comments}
									...
									{foreach $comments as $comment}
										...
										<div class="comment">
											...
											<p class="comment__text">{$comment->text|escape|nl2br|hashtag}</p>
										</div>
									{/foreach}
                                {/if}
						...
						<div data-accordion-tab="3" class="accordion__tab accordion__tab__3 map">
							<div class="accordion__tab__wrapper">
								<div class="cart__deliverie__shops">
									...
									<div class="cart__deliverie__shops__title title_for_attention">
										<i class="fa fa-map-marker" aria-hidden="true"></i>
										<div>Выберите магазин или пункт выдачи</div>
										<p class="attention">Внимание! Фактическое наличие не учитывает товары, проданные в течение дня, поэтому чтобы быть уверенным в наличии товара(ов) в нужном Вам магазине - забронируйте его (их).</p>
									</div>

									<div class="cart__deliverie__shops__body list_for_attention">
		...
		{if $category->name == "Самокаты для детей"}
		    <h2>Как подобрать самокат по росту</h2>
		    <img src="/files/uploads/kak-vibirat-samokat.jpg" alt="Как выбрать самокат" class="categoryImg" id="accordion__delivery_img"> <br><br>
		    <a href="/stock/kak-vybrat-samokat"   class="card__how">Посмотреть всю статью</a> <br><br><br>
		{/if}
		
		{if $category->name == "2-х и 4-х колесные" 
            || $category->name == "3-х колесные" 
            || $category->name == "Беговелы" 
            || $category->name == "Бибикары"
            || $category->name == "Подростковые"}
            <h2>Как подобрать велосипед по росту</h2>
		    <img src="/files/uploads/kak-vibirat-babybike1.jpg" alt="Как выбрать велосипед" class="categoryImg" id="accordion__delivery_img"> <br><br>
		    <a href="/stock/kak-vybrat-detskij-velosiped"   class="card__how">Посмотреть всю статью</a> <br><br><br>
        {/if}
        
        {if $category->name == "Женские" 
            || $category->name == "Городские и дорожные" 
            || $category->name == "Горные"}
            <h2>Как подобрать велосипед по росту</h2>
		    <img src="/files/uploads/kak-vzroslij-vibirat-bike.jpg" alt="Как выбрать велосипед" class="categoryImg" id="accordion__delivery_img"> <br><br>
		    <!-- <a href="/stock/kak-vybrat-detskij-velosiped"   class="card__how">Посмотреть всю статью</a> <br><br><br> -->
        {/if}
		{if $related_products}
