...
{if !$order->paid}
	...
	{if $payment_methods && !$payment_method && $order->total_price>0}
		<form method="post">
			...
			<input type='submit' class="button my_btn_1" value='Закончить заказ'>
	...
	{elseif $payment_method}
		<h2>Способ оплаты &mdash; {$payment_method->name}
			<form method=post><input type=submit name='reset_payment_method' value='Выбрать другой способ оплаты' class="my_btn_1"></form>	
		</h2>
		
		<p>{$payment_method->description}</p>
		
		<h2>К оплате {$order->total_price|convert:$payment_method->currency_id}&nbsp;{$all_currencies[$payment_method->currency_id]->sign}</h2>
