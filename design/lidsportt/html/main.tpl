			...
			$(function() {
				$('.home_slide_in').carouFredSel({
						...
						<ul class="home_slide_in">
                                	<li>
                            			<a href="http://lidsport.ru/dsk-i-aksessuary"><img src="design/{$settings->theme}/images/ban0.png" alt=""></a>
                             	   </li>
                                	<li>
                ...
                -->  
                  ...
                  {* Новинки *}
                 {get_new_products var=new_products limit=15 visible=1}
                 {if $new_products} 
                  {literal}
					<script type="text/javascript">
                    $(function() {
							...
							height: 322,
							/*scroll: {fx:'fade'},*/
							prev: {button:'#prev3',items: 1},
							next: {button:'#next3',items: 1},
							/*auto:{play: false},*/
                    ...
                    {/literal} 
                  <div class="div_leaders mb30">
                  <i class="nhjk">Новинки</i>
                  
                  	<a href="javascript:void(0)" class="leaf fl mt140 ml5 prev2" id="prev3"></a>
                      <ul class="ul_leaders" id="slider_2">
                        {foreach $new_products as $product}
                          <li> 
                            <script>
                            console.log("{$product->cat_url} ddddd");
                            </script>
                          	<a href="{$product->cat_url}/u_{$product->url}">
                                    <span class="block_img">
                                     {if $product->imagesc}
                                  ...
                                  {if $product->variants|count < 2 && $product->variants|count != 0}
                                            ...
                                            <form class="variants" action="/cart">
                                                 ...
                                                 <div class="out-of-stock" style="margin-top: -22px;">
                                                  ...
                                                  {else}<input id="variants_{$v->id}" name="variant" value="{$v->id}" type="radio" class="variant_radiobutton" {if $v@first}checked{/if} style="display:none;"/>
                                                 <span class="relatedPrice fl">{if $v->compare_price > 0}<span class="discountPriceB"><span class="tdlt c_f00">{$v->compare_price|convert}</span><span class="rub b mt-1 pr"><span class="r_da pa">-</span>P</span>&nbsp;&nbsp;</span>{/if}{$v->price|convert} <span class="rub b mt-1 pr"><span class="r_da pa">-</span>P</span></span><span style="position: relative;"> 
                                                <input type="submit" class="carts_b" data-result-text="добавлено"></input><span class="buyButton">КУПИТЬ</span></span>
                                            </form>
                                                {/if}{/foreach}
                                            <!-- Выбор варианта товара (The End) -->
                                            {elseif $product->variants|count >1}{foreach $product->variants as $v}{if $v->pod_zakaz == 1}
                                                 <div class="out-of-stock" style="margin-top: -22px;">
                                                  ...
                                                  {else}<input id="variants_{$v->id}" name="variant" value="{$v->id}" type="radio" class="variant_radiobutton" {if $v@first}checked{/if} style="display:none;"/>
                                                 <span class="relatedPrice fl">{if $v->compare_price > 0}<span class="discountPriceB"><span class="tdlt c_f00">{$v->compare_price|convert}</span><span class="rub b mt-1 pr"><span class="r_da pa">-</span>P</span>&nbsp;&nbsp;</span>{/if}{$v->price|convert} <span class="rub b mt-1 pr"><span class="r_da pa">-</span>P</span></span>
                                                <a href="{$product->cat_url}/u_{$product->url}" style="position: relative;" class="data" product="{$product->id}">{/if}<input type="submit" class="carts_b" data-result-text="добавлено"></input><span class="aboutButton" product="{$product->id}">КУПИТЬ</span></span>{break}{/foreach} 
                                                <!--<a href="{$product->cat_url}/u_{$product->url}" class="more_btn h30 br3 min_btn trable1 m0a w100 db lh29">Подробнее</a>-->
                                                {else}
                                            <div class="out-of-stock" style="margin-top: -22px;">
                                                    <span style="font-size: 20px;">Под заказ</span>
                                                    <span style="display: block;font-size: 14px;">цены уточняйте у продавцов-консультантов</span>
                      ...
                      <a href="javascript:void(0)" class="leaf2 mt140 mr5 next2" style="z-index:1;" id="next3"></a> 
                      <div class="hidden_line"></div> 
                  </div>
                  {/if}
