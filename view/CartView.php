...
//    	$order->shop_id     = $this->request->post('shop_id');
...
//        $this->design->assign('shop', $shop);
    	...
    	elseif($_SESSION['captcha_code_cart'] != $captcha_code || empty($captcha_code))
    	{
    		
			$this->design->assign('captcha', $_SESSION['captcha_code_cart']);
    	...
    	else
    	{
	    	...
	    	{
                $product_id = $this->variants->get_variant($variant_id)->product_id;
	    	    $this->orders->add_purchase(array('order_id'=>$order_id, 'variant_id'=>intval($variant_id), 'amount'=>intval($amount), 'sale_price'=>$cart->discount_text[$product_id]));
	    	}
