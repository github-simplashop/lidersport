...
class View extends Simpla
{
	...
	public function __construct()
	{
		...
		else
		{
			...
			// Отключено, вместо ipgeobase установлено определение города Yandex
//			require_once('geo.php');
//			$o = array(); // опции. необзятательно.
//			$o['charset'] = 'utf-8'; // нужно указать требуемую кодировку, если она отличается от windows-1251
//
//			$geor = new Geor($o); // запускаем класс
//			$city = $geor->get_value('city', true);
//			$this->design->assign('city', $city);
  
			// Настраиваем плагины для смарти
			$this->design->smarty->registerPlugin("function", "get_productst",   array($this, 'get_productst_plugin'));
			$this->design->smarty->registerPlugin("function", "get_categories_parent",   array($this, 'get_categories_parent'));
			$this->design->smarty->registerPlugin("function", "get_posts",               array($this, 'get_posts_plugin'));
			$this->design->smarty->registerPlugin("function", "get_news",                array($this, 'get_postsnew_plugin'));
            ...
            $this->design->smarty->registerPlugin("function", "get_url_params",          array($this, 'get_url_params_plugin'));
		}
	}
	...
	public function get_postsnew_plugin($params, &$smarty)
	{
		if(!isset($params['visible']))
			$params['visible'] = 1;
		if(!empty($params['var']))
	...
	public function get_categories_parent($params, &$smarty)
	{
		// $cts = $this->categories->get_categories_parent($params['id']);
		if ($cts) 	{}
		else {$cts = $this->categories->get_categories_parent($params['parent_id']);}
		$smarty->assign($params['var'], $cts);
	}
	 ...
	 *
	 */	
	 public function get_productst_plugin($params, &$smarty)
	{
		if(!isset($params['visible']))
			$params['visible'] = 1;
    ...
    public function get_url_params_plugin()
    {
        $actual_link = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        $params      = explode('?', $actual_link)[1];
